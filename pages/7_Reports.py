...
def get_ranked_players(data_dir: str, mtimes: tuple, weights_mtime: float) -> pd.DataFrame:
    ...
    get_ranked_players() for the same fix applied there first. Also
    missing until now: this page never actually read the saved source
    weights at all, so its numbers could silently disagree with the
    Draft Board's even with the cache-key bug fixed."""
